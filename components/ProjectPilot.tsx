...
export const ProjectPilot: React.FC = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const combinedInput = input ? `${input}\n\n${attachmentMsg}` : attachmentMsg;

      // Trigger send automatically or just append to input? 
      // Let's append to input so user can explain it, OR just send. 
      // Better: Send immediately as a user message.
      await sendToPilot(combinedInput);
  ...
  const handleSend = () => sendToPilot(input);
    ...
    <div className="pb-8 pt-4 sticky bottom-0 bg-black/80 backdrop-blur-lg border-t border-white/5 -mx-4 px-4 md:mx-0 md:px-0">
      <div className="relative group max-w-3xl mx-auto flex items-center gap-3">
        ...
        <button
          ...
          className="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-white/60 hover:text-white modern-transition"
        >
          📎
        </button>
        <input
