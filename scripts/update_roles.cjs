...
async function run() {
    try {
      ...
      add column if not exists role text default 'homeowner' check (role in ('homeowner', 'vendor', 'admin'));
    `;

        await client.query(sql);
        console.log('Schema updated successfully: add column role with check constraint');
    } catch (err) {
